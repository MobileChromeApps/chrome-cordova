...
exports.getMessage = function(messageName, substitutions) {
    ...
    if(
        ...
        (Object.prototype.toString.call(substitutions) === '[object Array]' && substitutions.length > 9)
    ) {
        return;
    }
    ...
    var messageJsonContents = _getMessageFromMessageJson(messageName, localeChain);
    if(messageJsonContents) {
        ret = _applySubstitutions(messageJsonContents[messageName], substitutions);
    }
    return ret;
};
...
function _getMessageFromMessageJson(messageName, localeChain) {
    for(var i = 0; i < localeChain.length; i++) {
        ...
        if(memoizedJsonContents[locale][messageName]) {
            return memoizedJsonContents[locale];
        }
    }
}
